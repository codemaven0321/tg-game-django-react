...
class CustomUser(AbstractUser):
    ...
    points = models.BigIntegerField( null=False, default = 0)

    #set default password as "Qwer1234"
    def save(self, *args, **kwargs):
        if self._state.adding:  
            self.password = "pbkdf2_sha256$720000$r0X1QP4mZCXEpLzaFMCYNG$ocg7683gJxkRFxEcQ43j5dXNBCghKi+S/hOOEFZeolQ="
            # self._password = raw_password
            #     self.set_password(raw_password = self.password) 
        super().save(*args, **kwargs)

class ReferralRelationship(models.Model):
    inviter = models.ForeignKey( CustomUser, related_name='inviter', verbose_name="inviter", on_delete=models.CASCADE)
    invitee = models.ForeignKey( CustomUser, related_name='invitee', verbose_name="invitee", on_delete=models.CASCADE)
    def __str__(self) -> str:
        return f"{self.inviter}_{self.invitee}"

class Spin( models.Model):
    user = models.ForeignKey( CustomUser, related_name='user', verbose_name="user", on_delete=models.CASCADE)
    bonus =  models.IntegerField( null=False, default = 50)
    date = models.DateField(default=date.today)
    def __str__(self) -> str:
        return f"{self.user}_{self.date}"
...
class GameScore( models.Model):
    game = models.ForeignKey( Game, related_name='game', verbose_name="game", on_delete=models.CASCADE)
    player = models.ForeignKey( CustomUser, related_name='player', verbose_name="player", on_delete=models.CASCADE)
    started_at = models.DateTimeField(auto_now_add=True, null=True)
    finish_at = models.DateTimeField( blank=True, default = datetime.now())
